...
The Pages workflow builds the TypeScript browser sources and validates all local HTML asset references on pull requests. Deployment runs only after merge to `main`, uploads only `web/`, and grants `pages: write` plus `id-token: write` solely to the deployment job.

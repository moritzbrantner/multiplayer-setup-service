...
fn lobby_session_supports_mesh_and_host_topologies() {
    ...
    assert!(entrypoint.contains("./resilient-lobby-session.ts"));
...
fn demo_lobbies_share_invites_chat_and_latency_telemetry() {
    ...
    for script in ["pong.ts", "tic-tac-toe.ts", "card-game.ts", "arena.ts"] {
        assert!(web_file(script).contains("./lobby-experience.ts"));
    }
    for page in [
        "pong.html",
        "tic-tac-toe.html",
...
fn pages_explainer_documents_the_actual_multiplayer_boundary() {
    ...
    assert!(index.contains("./explainer.ts"));

    assert!(explainer.contains("TURN may relay"));
    assert!(explainer.contains("import { topologyEdgeCount } from \"./arena-model.ts\""));
...
fn pages_client_code_components_show_supported_browser_apis() {
    ...
    assert!(index.contains("./client-code-components.ts"));

    assert!(components.contains("import { PeerSession } from \"./session.ts\""));
    assert!(components.contains("import { LobbySession } from \"./lobby-session.ts\""));
